<template>
  <div class="app-container">
    <div class="entry-path-tip">
      ...
      <span>系统设置 → 客户菜单功能管理</span>
      <span class="divider">|</span>
      <span class="label">访问地址：</span>
      <span>/system/customerMenuManage</span>
    </div>
      ...
      <el-table-column label="操作" width="260" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['sb:system:customerMenuManage:edit']"
          ...
          <el-button
            ...
            v-hasPermi="['sb:system:customerMenuManage:query']"
          >启停用记录</el-button>
          <el-button
            size="mini"
            type="text"
            icon="el-icon-time"
            @click="handlePeriodLog(scope.row)"
            v-hasPermi="['sb:system:customerMenuManage:query']"
          >启停用时间段</el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="statusDialogTitle" :visible.sync="openStatus" width="480px" append-to-body>
      <el-form ref="statusForm" :model="statusForm" :rules="statusRules" label-width="80px">
        <el-form-item label="功能名称">
          <el-input v-model="statusForm.menuName" disabled />
        </el-form-item>
    ...
    <el-dialog title="启停用时间段" :visible.sync="openPeriod" width="720px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="openPeriod = false">关 闭</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import {
  listMenusByCustomerId,
  changeMenuStatus,
  getMenuStatusLogs,
  getMenuPeriodLogs
} from '@/api/system/customerMenuManage'

export default {
  name: 'CustomerMenuManage',
  data() {
    return {
      selectedCustomerId: '',
      customerOptions: [],
      ...
      openLog: false,
      openPeriod: false,
      statusForm: { customerId: '', customerName: '', menuId: '', menuName: '', status: '0', reason: '' },
      statusRules: {
        reason: [{
          required: true,
  ...
  methods: {
    ...
    onCustomerChange() {
      ...
      this.loading = true
      listMenusByCustomerId(this.selectedCustomerId).then(res => {
    ...
    submitStatusForm() {
      this.$refs.statusForm.validate(valid => {
        if (!valid) return
        changeMenuStatus({
          customerId: this.statusForm.customerId,
          menuId: this.statusForm.menuId,
    ...
    handleStatusLog(row) {
      this.logForm = { menuName: row.menuName }
      this.openLog = true
      this.statusLogList = []
      getMenuStatusLogs(row.customerId, row.menuId).then(res => {
    ...
    handlePeriodLog(row) {
      ...
      this.periodLogList = []
      getMenuPeriodLogs(row.customerId, row.menuId).then(res => {

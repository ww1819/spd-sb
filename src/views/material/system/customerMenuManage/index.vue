      ...
      <el-table-column label="操作" width="260" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['hc:system:customerMenuManage:edit']"
          ...
          <el-button
            ...
            v-hasPermi="['hc:system:customerMenuManage:query']"
          >启停用记录</el-button>
          <el-button
            size="mini"
            type="text"
...
import {
  ...
  getMenuStatusLogs,
  getMenuPeriodLogs
} from '@/api/material/customerMenuManage'
  ...
  methods: {
    ...
    onCustomerChange() {
      ...
      listMenusByCustomerId(this.selectedCustomerId).then(res => {
        const list = res.data || []
        list.forEach(item => { item.customerId = item.customerId || item.tenantId || this.selectedCustomerId })
        this.menuList = list
        this.loading = false
    ...
    submitStatusForm() {
      this.$refs.statusForm.validate(valid => {
        if (!valid) return
        changeMenuStatus({
          tenantId: this.statusForm.customerId,
    ...
    handleStatusLog(row) {
      ...
      this.openLog = true
      this.statusLogList = []
      getMenuStatusLogs(tenantId, row.menuId).then(res => {
    ...
    handlePeriodLog(row) {
      ...
      this.periodLogList = []
      getMenuPeriodLogs(tenantId, row.menuId).then(res => {
